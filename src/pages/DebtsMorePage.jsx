...
const DebtsMorePage = ({ totalAmounts }) => {
  let { debtsId } = useParams();
  let amount = totalAmounts.find((totalAmount) => totalAmount.id === debtsId);
  ...
  return (
    ...
    <Card style={{ width: '40rem',padding:"20px" }}>
      ...
      <Card.Body>
        <Link to="/debts" style={{textDecoration:"none", width:"100%",background:"blue",display:"block",color:"white",paddingTop:"10px",paddingBottom:"10px",textAlign:"center",borderRadius:"40px"}}>Back to home</Link>
      </Card.Body>
    </Card>
  </div>
  )
